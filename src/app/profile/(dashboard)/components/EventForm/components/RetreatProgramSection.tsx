...
export const RetreatProgramSection = ({
  control,
  register,
  errors,
  programFields,
  ...
  const getProgramDayTitle = (index: number) => {
    if (!dateRange?.from) {
      return `Dzień ${index + 1}`;
    }

    return format(addDays(dateRange.from, index), "d MMMM (EEEE)", { locale: pl });
  };
        ...
        {programFields.map((field, index) => (
          <div key={field.id} className="border-b pb-6 last:border-b-0">
            <div className="flex items-center justify-between">
              <Label className="text-lg font-semibold">{getProgramDayTitle(index)}</Label>
              <Button
                type="button"
                variant="ghost"
                size="sm"
