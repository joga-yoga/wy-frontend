...
export function EventHelpBar({ mode = "retreat" }: { mode?: "retreat" | "workshop" }) {
  const {
    activeTipId,
  ...
  if (isMobile) {
    const activeTip =
      mode === "retreat"
        ? tipsRetreats.find((tip) => tip.id === activeTipId)
        : tipsWorkshops.find((tip) => tip.id === activeTipId);
    return (
