 ...
 *
 * Visual language deliberately echoes `GrafikSessionCard` (the session row on the Grafik page)
 * — rounded-xl bordered card, `w-1` colour bar, instructor avatar+name row, status badges — so
 * a session looks like the same object whether you're looking at it in the list or acting on it
 * here. Two differences from that card: no chevron (this card is never itself tappable — it's
 * a static context header) and no slots/fill count (not relevant to "what am I editing"). It
 * adds one thing the list doesn't need: an explicit date, since unlike Grafik's day-grouped
 * list, this card is shown standalone with no day header to imply it.
 */
export function SessionContextCard({
  ...
  title: string;
  /** Shown as a small eyebrow line above the title, e.g. "Śro. 12 sierpnia". */
  date?: string | null;
  ...
  return (
    <div
      className={cn(
        "flex gap-3 rounded-xl border-[1.5px] bg-white px-3 py-2.5",
        tone === "danger" ? "border-b2b-red-border bg-b2b-red-bg" : "border-border",
      )}
    >
      <div
        className={cn(
